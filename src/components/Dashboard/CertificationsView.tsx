...
import { FileCheck, Plus, Search, Download, Award, Trash2, Eye, X } from 'lucide-react';
...
const CertificationsView: React.FC = () => {
    ...
    const handleDelete = async (id: string) => {
        if (window.confirm('Tem certeza que deseja excluir esta certificação? Esta ação não pode ser desfeita.')) {
            try {
    ...
    const handleExport = () => {
        const headers = ["ID", "Nome", "Agricultor", "Nível", "Status", "Validade"];
        const rows = certifications.map(c => [
            ...
            c.nivel,
            c.status,
            c.validade ? new Date(c.validade).toLocaleDateString('pt-BR') : 'N/A'
        ]);
        ...
        link.setAttribute("download", "certificacoes_export.csv");
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };

    const generatePdfDoc = (cert: any) => {
        const doc = new jsPDF({
        ...
        doc.text(`NÍVEL ${cert.nivel?.toUpperCase() || 'BRONZE'}`, 148.5, textY + 60, { align: "center" });

        // Date and Validity
        doc.setFont("times", "italic");
        doc.setFontSize(12);
        const createdAt = cert.createdAt?.seconds ? new Date(cert.createdAt.seconds * 1000) : new Date();
        const validade = cert.validade ? new Date(cert.validade) : new Date();

        doc.text(`Emitido em: ${createdAt.toLocaleDateString('pt-BR')}`, 148.5, textY + 80, { align: "center" });
        doc.text(`Válido até: ${validade.toLocaleDateString('pt-BR')}`, 148.5, textY + 88, { align: "center" });
    ...
    const handlePreview = (cert: any) => {
        ...
        setPreviewPdfUrl(pdfBlob.toString());
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h3 className="text-2xl font-bold text-stone-800 flex items-center gap-2">
                        <Award className="text-green-600" /> Gestão de Certificações
                    </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-100">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded-lg">Total Ativo</span>
                    </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-100">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <span className="text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded-lg">Crítico</span>
                    </div>
                            ...
                            {filtered.map(c => (
                                <tr key={c.id} className="hover:bg-stone-50/50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div className="text-xs text-stone-500">Auditor: {c.auditor || "Sistema"}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="bg-white w-full max-w-5xl h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-scale-in">
                        <div className="flex justify-between items-center p-4 border-b border-stone-100 bg-stone-50">
                            <h3 className="text-lg font-bold text-stone-800 flex items-center gap-2">
                                <Eye size={20} className="text-blue-600" /> Visualização do Certificado
                            </h3>
                            <button
                        ...
                        <div className="flex-1 bg-stone-100 p-4">
                            <iframe src={previewPdfUrl} className="w-full h-full rounded-xl shadow-lg border border-stone-200" title="PDF Preview"></iframe>
                        </div>
                        <div className="p-4 border-t border-stone-100 flex justify-end gap-3 bg-white">
                            <button onClick={() => setIsPreviewOpen(false)} className="px-5 py-2.5 text-stone-600 hover:bg-stone-50 rounded-xl transition-colors font-medium">Fechar</button>
                        </div>
                    </div>
                </div>
            )}
